...
def get_output_layers(net):
    ...
    # OpenCV 3.x returns [[200], [227], [254]]; OpenCV 4.x returns [200, 227, 254]
    if hasattr(unconnected, 'shape') and len(unconnected.shape) > 1:
        indices = [idx[0] - 1 for idx in unconnected]
    else:
        indices = [int(idx) - 1 for idx in unconnected]
    return [layer_names[i] for i in indices]
